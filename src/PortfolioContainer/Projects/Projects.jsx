...
const Projects = () => {
  ...
  const [slicedRandomProj, setSlicedRandomProj] = useState([]);
  ...
  useEffect(() => {
    if (randomProj.length > 4) {
      setSlicedRandomProj(randomProj.slice(0,4));
    } else {
      setSlicedRandomProj(randomProj);
    }
  }, [])
  ...
  return (
    <section id="anchor-projects">
      ...
      <div className="container cards">
        {slicedRandomProj.map((hej, index) => {
          return(  
      ...
      {/* { !showMore ?
        initRandomProject(randomProj.slice(0,4))
        : 
        initRandomProject(randomProj)
      } */}

      <div style={{ display: (randomProj.length > 4) ? 'flex' : 'none' }} className="toggle__grid">
        { !showMore ? 
          
          (<p className="toggle__grid-expand" onClick={() => {
            setSlicedRandomProj(randomProj);
            setShowMore(true);
          }}> 
            Show More
